...
function ChatInterface() {
  ...
  return (
    <div className="flex h-screen w-full bg-[#0a0a0b] text-gray-200 overflow-hidden font-sans selection:bg-purple-500/30">
      {/* Sidebar - Improved Glassmorphism */}
      <aside className="w-72 glass border-r border-white/5 flex flex-col h-full z-20">
        <div className="p-6">
          <button 
            onClick={clearChat}
            className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 hover:border-purple-500/50 transition-all duration-300 text-sm font-medium group"
          >
      ...
      <main className="flex-1 flex flex-col h-full relative z-10">
        ...
        {/* Header - Glass Effect */}
        <header className="h-20 border-b border-white/5 flex items-center px-8 justify-between glass z-20">
          <div className="flex items-center gap-3">
            ...
            <div>
                <h2 className="text-xl font-bold text-white tracking-tight">Ervis</h2>
                <div className="flex items-center gap-1.5 text-[10px] text-green-500 font-medium uppercase tracking-tighter">
          ...
          <div className="flex gap-4">
            <button className="w-10 h-10 rounded-xl flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/5 transition-all">
              <Github size={20} />
            </button>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-12 space-y-8 scrollbar-thin">
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center px-4">
              <div className="w-24 h-24 rounded-[2rem] bg-gradient-to-tr from-purple-600/20 to-indigo-600/20 flex items-center justify-center mb-8 border border-white/5 glass-card animate-pulse">
                <Bot size={56} className="text-purple-400" />
              </div>
              <h1 className="text-5xl font-black mb-4 bg-gradient-to-b from-white to-gray-500 bg-clip-text text-transparent">Ervis'e Merhaba De</h1>
              ...
              <div className="mt-12 flex gap-3 flex-wrap justify-center">
                  {['"Bugün hava nasıl?"', '"Kiralık ev fiyatları?"', '"Maç sonucu?"'].map(s => (
                      <button key={s} onClick={() => setInput(s.replace(/"/g, ''))} className="px-5 py-2.5 rounded-2xl glass-card text-sm text-gray-400 hover:border-purple-500/30 hover:text-purple-400 transition-all font-medium">
                          {s}
                      </button>
                  ))}
              </div>
        ...
        <div className="p-8 md:p-12 relative z-20">
          ...
          >
            ...
            <div className="relative glass rounded-[2rem] border border-white/10 shadow-2xl p-2 flex items-center group focus-within:border-purple-500/50 transition-all duration-500">
                <div className="pl-4 pr-2 text-gray-500 group-focus-within:text-purple-400 transition-colors">
                    <Command size={22} />
                </div>
                <input 
                  type="text" 
                  ...
                  placeholder="Ervis'e bir talimat ver veya soru sor..."
                  disabled={isLoading}
                  className="flex-1 bg-transparent border-none px-4 py-4 text-white placeholder-gray-600 focus:outline-none focus:ring-0 text-lg font-light transition-all disabled:opacity-50"
                />
                <button 
                  type="submit"
                  disabled={!input.trim() || isLoading}
                  className="ml-2 w-14 h-14 bg-purple-600 hover:bg-purple-500 disabled:bg-gray-800 disabled:opacity-30 rounded-2xl text-white transition-all shadow-lg flex items-center justify-center shrink-0 active:scale-95 glow-hover"
                >
                  <Send size={24} />
                </button>
            </div>
