...
import { canAccessEmployee } from '@/server/auth/access';
...
export async function POST(req: NextRequest) {
  ...
  const employee = await prisma.employee.findUnique({ where: { id: employeeId }, select: { id: true, organizationId: true } });
  ...
  const source = await prisma.contextSource.create({
    data: {
      ...
      ownerType: ContextOwnerType.EMPLOYEE,
      ownerId: employee.id,
      title: file.name,
  ...
  await createAuditLog({
    ...
    subjectId: source.id,
    metadata: { ownerType: 'EMPLOYEE', ownerId: employee.id, fileName: file.name },
  });
