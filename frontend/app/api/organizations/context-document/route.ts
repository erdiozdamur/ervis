...
export async function POST(req: NextRequest) {
  ...
  if (!(await canAccessOrganization(session.user.id, organizationId))) return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
  ...
  const source = await prisma.contextSource.create({
    data: {
      organizationId,
      ownerType: ContextOwnerType.ORGANIZATION,
